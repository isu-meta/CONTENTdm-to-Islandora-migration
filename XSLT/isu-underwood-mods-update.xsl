...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:saxon="http://saxon.sf.net/"
    ...
    <xsl:template match='//mods:genre[@authority="dct"]' exclude-result-prefixes="#all">
        ...
        <xsl:variable name="type_of_resource">
            <xsl:choose>
                ...
                <xsl:otherwise>
                    <!-- 
                        In the case of the Underwood collection, if the DC type
    ...
    <xsl:template match="//mods:title" exclude-result-prefixes="#all">
        <!--
        Remove double-quotes from a tag's text.
        
        CONTENTdm has an odd behavior of adding quote marks around quote
        marks, at least in the title field. This results in titles that
        contain parts like this '&amp;quot;&amp;quot;Me Sister Annie,
        &amp;quot;&amp;quot;' in the preprocessed files generated by the 
        CONTENTdm to MODS Staging module in Islandora.
        
        This template replaces '&amp;quot;&amp;quot;' with a single double
        quote ('"').
        -->
        <xsl:variable name="title_text" select="text()" />
        <xsl:variable name="quote_string">"</xsl:variable>
        <xsl:value-of select="replace($title_text,'&amp;quot;&amp;quot;', $quote_string)" />
    </xsl:template>

...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:saxon="http://saxon.sf.net/"
    ...
    <xsl:template match='//mods:genre[@authority="dct"][text() != ""]' exclude-result-prefixes="#all">
        ...
        The objects in the Harker collection are all Image type, so the only 
        thing we need to do is to copy the existing "dct" genre element and add
        a still image typeOfResource element.
        -->
        <xsl:copy>

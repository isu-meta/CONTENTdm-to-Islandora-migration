...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:saxon="http://saxon.sf.net/"
    ...
    <xsl:template match='//mods:genre[@authority="dct"]' exclude-result-prefixes="#all">
        ...
        <xsl:variable name="type_of_resource">
            <xsl:choose>
                ...
                <xsl:otherwise>
                    <!-- 
                        In the case of the President collection, if the DC type
    ...
    <xsl:template match="mods:title | cdm:title | mods:abstract | cdm:description" exclude-result-prefixes="#all">
        ...
        Some titled and descriptions include unnecessary double quotes like
        '&quot;&quot;Achieving Excellence&quot;&quot;' that should be single
        quotes.
        
        This template replaces any number of '&quot;' repetitions with a single
        double quote ('"').
        -->
    ...
    <xsl:template match="mods:topic | cdm:topical-subject-lcsh" exclude-result-prefixes="#all">
        <!-- 
        Remove "Iowa State University;" and "Iowa State College;" from the
        Subject (Topics) fields since they are not drawn from LCSH, the
        authority governing the field and are duplicative of the metadata in
        the Subject (Names) fields.
        -->
        <xsl:variable name="topic_txt" select="text()" />
        <xsl:copy>
            <xsl:value-of select="replace($topic_txt,'Iowa State (University|College); *', '')" />
        </xsl:copy>
    </xsl:template>
</xsl:stylesheet>